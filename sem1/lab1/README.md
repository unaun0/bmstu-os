...
- В DOS, при вызове прерывания, процессор автоматически сохраняет флаги и IP на стеке, но это происходит до вызова обработчика прерывания. Это связано с тем, что обработчик прерывания int 8h является частью ядра DOS и должен работать быстро и эффективно.
...
- Запрещает маскируемые прерывания. Если бы прерывания не были запрещены, то другой обработчик прерывания мог бы прервать выполнение операций с системным таймером, что могло бы привести к сбою.
...
- Это связано с тем, что при косвенном вызове прерывания через адрес в памяти (как в строке 38: FF 1E 0070 call dword ptr es:[70h]), флаги не сохраняются на стеке автоматически, потому что косвенный вызов прерывания является обычным вызовом подпрограммы.
