...
Таблицы семафоров содержат дескрипторы наборов семафоров (у каждого набора семафоров в этой таблице есть дескриптор).
...
2. UID создателя набора семафоров и ID его группы. Процесс, UID которого совпадает с UID создателя набора семафора, может удалять набор и изменять его управляющие параметры.
...
Задача производства - потребления - задача, в которой не только осуществляется монопольный доступ к разделяемому ресурсу, который является буфер, но и реализуется синхронизация процессов.
