...
__Системный вызов fork()__ создает новый процесс, который является копией процесса - предка: процесс-потомок наследует адресное пространство процесса - предка, дескрипторы всех открытых файлов и сигнальную маску.
...
В первом варианте используется `sleep()`; во втором - `pause()`, т.e. нужно будет завершить дочерний процесс, который находится в прерываемом сне, через терминал (в примере - первый дочерний).
