...
- Нужно сохранять флаги перед косвенным вызовом, так как внутри обработчика прерывания есть инструкция IRET(Interupt Return), которая восстанавливает из стека флаги, а так же адрес возврата. В качестве адреса возврата она восстанавливает IP для ближнего перехода, для дальнего CS и IP.
...
- Делает следующуюю команду неделимой. Команда lock блокирует локальную шину памяти, чтобы ни одна команда ДО не смогла обратиться к памяти, потому что AND обращается к памяти два раза. 