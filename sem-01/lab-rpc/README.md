...
__Структура BAKERY__

1. `num`: номер в очереди, который выдаётся клиенту.
2. `pid`: идентификатор клиента (процесса).
3. `result`: результат выполнения операции.

__Программа BAKERY_PROG__

1. Определяет RPC-программу с уникальным номером 0x20000001.
2. Содержит версию BAKERY_VER, где описаны три удалённые процедуры: `GET_NUMBER` - получение номера в очереди; `WAIT_QUEUE` - ожидание в очереди; `BAKERY_RES` - получение результата выполнения операции.

__Версия и номера процедур__

У каждой процедуры уникальный номер: 1, 2, 3.
