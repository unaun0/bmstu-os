...
static ssize_t 
fortuneWrite
(struct file *file, const char __user *buf, size_t len, loff_t *fPos) {
...
// не меньшк 3х ид процессов - те, которые есть всегда 1 eho1, 2 eho2, 3 who3
// pos демострирует номер итерации

static ssize_t 
fortuneRead
(struct file *file, char __user *buf, size_t len, loff_t *fPos) {
  int readLen;
